...
def metadata_date_key(metadata, pair):
    fromisoformat = datetime.fromisoformat(
        metadata[pair[0]].get("date", datetime.now().isoformat())
    )
...
class Engine:
    ...
    def _produce(self, filename, destination, page=None):
        ...
        with open(path) as f:
            self.metadata[filename], file_content = strip_file_meta(
                path, copy.deepcopy(self.meta_categories), self.toc_condition
            )
            ...
            with (
                open(os.path.join(destination, html_name(filename)), "w") as f,
    ...
    def generate(self):
        ...
        listing = sorted(
            os.listdir(self.source_dir), key=birthtime_for_filename, reverse=True
        )

        article_count = len(listing)
        page_count = article_count // self.articles_per_page + (
            0 if not article_count % self.articles_per_page else 1
        )

        if article_count > self.articles_per_page:
            self.log(
                f"⚠️ Too many articles! {article_count} articles will be broken up into {page_count} pages ",
                level=EBLogLevel.WARN,
            )
            self.file_guard(
                self._build_paginated_articles,
                page_count,
                self.kPageUrlFmt,
                listing,
            ...
            self.file_guard(
                ...
                listing,
                self.public_dir,
                action_description="building articles",
            )

        self.log("📄 Creating index file")
        self.file_guard(
            self._build_index, self.index_content, action_description="building index"
        )
        ...
        self.file_guard(
            self._write_search_template, action_description="writing search template"
        )
    ...
    def _clean_build_dir(self):
        if os.path.exists(self.public_dir) and not os.path.isdir(self.public_dir):
            raise EngineBuildError(
                '‼️ specified public director "{}" is not a directory'.format(
                    self.public_dir
                )
            )
        if not os.path.exists(self.public_dir):
            self.log(
                "⚠️ Cleaning public dir specified, but no public dir found",
                level=EBLogLevel.WARN,
            )
        else:
            self.file_guard(
                shutil.rmtree,
                self.public_dir,
                action_description="removing existing files",
            )
        self.log("📂 Re-creating public dir")
        self.file_guard(
            os.mkdir, self.public_dir, action_description="Re-creating public dir"
        )

    def _write_search_template(self):
        with (
            open(os.path.join(self.private_dir, "search.html.template")) as f,
    ...
    def _write_metadata(self):
        serialize(
            {
                filename: {key: value.result for key, value in meta.items()}
    ...
    def _build_paginated_articles(self, dir_count, kPageString, listing):
        ...
        for i in range(dir_count - 1):
            ...
            current_page = self._render_paged_article(
                current_page, i, kPageString, slice
            )

        self._render_paged_article(
            current_page,
            dir_count - 1,
            kPageString,
